...
export default function Sessions({ rows, onOpen }) {
  ...
  /* Tokens and elapsed time attach to a session, not to a single action:
     ...
     are simply that session's own figures. */
  const resource = useMemo(() => {
    const ids = new Set(matched.map((r) => r.session_id).filter(Boolean));
    const inSessions = session !== "all" ? scoped
    ...
    inSessions.forEach((r) => {
      ...
      perDay[d].value += Number(r.total_tokens) || 0;
    });
    ...
    const owner = new Map();
    inSessions.forEach((r) => {
      if (r.session_id && !owner.has(r.session_id))
        owner.set(r.session_id, AGENT[r.tool] || "Unattributed");
    });
    const hoursBy = {};
    bounds.forEach(([a, b], id) => {
      const k = owner.get(id) || "Unattributed";
      hoursBy[k] = (hoursBy[k] || 0) + (b - a) / 36e5;
    });

    /* latency is a property of the action, so it needs no session proxy */
    const durations = inSessions.map((r) => Number(r.tool_duration_ms))
      .filter((n) => n > 0).sort((a, b) => a - b);
    const latencyBy = {};
    inSessions.forEach((r) => {
      const d = Number(r.tool_duration_ms);
      if (!(d > 0) || !r.tool_name) return;
      latencyBy[r.tool_name] ||= { n: 0, t: 0 };
      latencyBy[r.tool_name].n++;
      latencyBy[r.tool_name].t += d;
    });

    const totalMinutes = [...bounds.values()]
      .reduce((a, [x, y]) => a + (y - x) / 6e4, 0);

    return {
      sessions: ids.size || (session !== "all" ? 1 : 0),
      ...
      stalls: cnt(inSessions, (r) => has(r.hung_seconds)),
      trend: Object.values(perDay).sort((a, b) => a.label.localeCompare(b.label)),
      ...
      latencyByTool: Object.entries(latencyBy)
        .map(([label, v]) => ({ label: label.slice(0, 20), value: Math.round(v.t / v.n) }))
        .sort((a, b) => b.value - a.value).slice(0, 10),
    };
  }, [matched, scoped, session]);
